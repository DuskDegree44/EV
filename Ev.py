...
def menu():
        ...
        if name == 1:
            ...
            luli = [sala1,sala2,sala3,sala4,sala5,sala6,sala7,sala8,sala9,sala10]
            come = []
            items = list(folio.items())
            df = pd.DataFrame(items, columns = ['Nombre','2'])
            df.to_csv('data.csv')
            listo = ''
            ...
            otro = listo.replace('[', 'p')
            etc = listo.replace('\xd8', 'p')
            lulin = listo.replace('', 'p')
            chiki = listo.replace(',', ';').replace('[', '').replace(']', '').replace("'", '').replace('"', '').replace('', '')
            
            escr = open('data.csv', 'w')
            escr.write(chiki)
            escr.close()
            print(listo)
            time.sleep(0)
